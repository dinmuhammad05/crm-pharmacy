...
const ProfilePage: React.FC = () => {
  const { t, i18n } = useTranslation(); // t funksiyasi va i18n obyektini oldik
  ...
  if (!admin) {
    return (
      <div className="p-4 md:p-10">
        <Alert message={t('profile.no_data')} type="warning" showIcon />
      </div>
    );
  }
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 -mt-20 sm:-mt-24">
        <Card className="rounded-2xl sm:rounded-3xl shadow-2xl border-none overflow-hidden bg-white/95 backdrop-blur-sm">
          <div className="flex flex-col items-center">
            <div className="relative mt-1 mb-4 transition-transform hover:scale-105 duration-300">
            ...
            <div className="text-center px-4 w-full">
              <Title
                level={2}
                className="mb-1! text-slate-800! text-xl sm:text-2xl md:text-3xl font-bold"
              >
                {admin.fullName}
              </Title>
              ...
              <div className="flex flex-wrap justify-center gap-2 mb-6">
                <Tag
                  color="indigo"
                  className="m-0 px-4 py-1 rounded-full font-bold border-none bg-indigo-50 text-indigo-600 flex items-center"
                >
                  <SafetyCertificateOutlined className="mr-1" /> {admin.role}
                </Tag>
                <Tag
                  color={admin.isActive ? 'green' : 'red'}
                  className="m-0 px-4 py-1 rounded-full font-bold border-none uppercase text-[10px] tracking-widest flex items-center"
                >
                  {admin.isActive
                    ? t('profile.status_active')
                    : t('profile.status_inactive')}
                </Tag>
              </div>
            ...
            <Row gutter={[16, 16]}>
              <Col xs={24} sm={12}>
                <div className="bg-slate-50/50 p-4 rounded-2xl border border-slate-100 hover:bg-white hover:shadow-md transition-all group">
                  <Text
                    type="secondary"
                    className="text-[10px] uppercase font-bold text-slate-400 tracking-widest mb-2 block group-hover:text-indigo-500"
                  >
              ...
              <Col xs={24} sm={12}>
                <div className="bg-slate-50/50 p-4 rounded-2xl border border-slate-100 hover:bg-white hover:shadow-md transition-all group">
                  ...
                  >
                    ...
                    {t('profile.label_security_lvl')}
                  </Text>
                  <Text
                    strong
                    className="text-indigo-600 block uppercase tracking-tighter"
                  >
                    {admin.role} {t('profile.access_suffix')}
                  </Text>
                </div>
              </Col>
            </Row>
        ...
        <div className="mt-8 bg-indigo-50 border border-indigo-100 rounded-2xl p-4 sm:p-5 flex flex-col sm:flex-row gap-4 items-center sm:items-start text-center sm:text-left">
          ...
          <div>
            <Text strong className="text-indigo-900 block text-base">
              {t('profile.security_note_title')}
            </Text>
            <Text className="text-indigo-600 text-sm leading-relaxed">
              {t('profile.security_note_desc')}
            </Text>
          </div>
        </div>
      </div>
